...
void main() async {
  ...
  final results = await quran.search(query: 'mercy', language: 'english', limit: 3);
  print('   Found ${results.length} verses containing "mercy"');
  for (var v in results) {
    print('   - ${v.surah.name} ${v.number}: ${v.textEnglish.substring(0, 50)}...');
  }
  print('');
  ...
  print('   Juz ${juz?.number}: ${juz?.start.surah}:${juz?.start.verse} to ${juz?.end.surah}:${juz?.end.verse}\n');
  ...
  print('   First: ${sajdaVerses.first.surahNameArabic} ${sajdaVerses.first.verseNumber} (${sajdaVerses.first.type})\n');

  // Example 7: Get Hizb information
  print('7. Get Hizb information:');
  final hizb = await quran.getHizb(1);
  print('   ${hizb?.displayName}: ${hizb?.start.surah}:${hizb?.start.verse} to ${hizb?.end.surah}:${hizb?.end.verse}\n');
